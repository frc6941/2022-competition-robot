...
public class GoalTrack {
    ...
    public synchronized boolean tryUpdate(double timestamp, Pose2d new_observation) {
        ...
        if (distance < Constants.VisionConstants.Turret.MAX_TRACKING_DISTANCE) {
    ...
    synchronized void pruneByTime() {
        double delete_before = Timer.getFPGATimestamp() - Constants.VisionConstants.Turret.MAX_GOAL_TRACK_AGE;
    ...
    synchronized void smooth() {
        if (isAlive()) {
            ...
            for (Map.Entry<Double, Pose2d> entry : mObservedPositions.entrySet()) {
                if (t_now - entry.getKey() > Constants.VisionConstants.Turret.MAX_GOAL_TRACK_SMOOTHING_TIME) {
                    continue;
                }
                ++num_samples;
    ...
    public synchronized double getStability() {
        return Math.min(1.0, mObservedPositions.size() / (Constants.VisionConstants.Turret.FRAME_RATE * Constants.VisionConstants.Turret.MAX_GOAL_TRACK_AGE));
    }
